...
a custom config overrides it, and the lunch retarget actually changes the tags produced.
"""
...
def test_ingest_uses_supplied_vocab(tmp_path):
    ...
    inbox.mkdir()
    (inbox / "note.md").write_text("# Lunch\nThe burrito is fresh and cheap.\n")
...
def test_lunch_example_retargets_tags():
    # the shipped lunch retarget tags notes with its OWN vocabulary, never the default's.
    base = REPO / "examples" / "lunch"
    notes = ingest(base / "inbox", vocab=load_tag_vocab(base))
    all_tags = {t for n in notes for t in n.tags}
    assert {"health", "cost", "value", "variety"} <= all_tags
