...
EXAMPLES = INCIDENTS / "cases" / "01-checkout-latency" / "inbox"


def test_ingest_examples():
    notes = ingest(EXAMPLES, vocab=load_tag_vocab(INCIDENTS))
    assert len(notes) == 6
    # the rollback memo is about reverting a deploy in the spike window -> change + reversibility
    rollback = next(n for n in notes if "redeploy" in n.body.lower())
    assert "reversibility" in rollback.tags
    assert "change" in rollback.tags
