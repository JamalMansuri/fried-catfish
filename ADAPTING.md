...
your sources — biotech, finance, legal, ops, anything — means editing two files and pointing the
CLI at them. If you find yourself editing Python to change the domain, stop: you've missed a seam.

There is a complete worked retarget in [`examples/biotech/`](examples/biotech/) — read it alongside
...
```yaml
# default (software/eng)            # retargeted (biotech)
auth: auth                          assay: assay
billing: billing            ──►     contaminat: contamination
migrat: migration                   gxp: regulatory
launch: launch                      validation: validation
```

Keep the keys as substrings (`migrat` catches *migrate/migration/migrating*; `contaminat` catches
*contaminate/contamination/contaminant*). Keep the tag set small — a dozen tags is plenty.
...
goal: Surface every claim that has to hold for an FDA / GxP auditor, and what breaks if it doesn't.
utility_fn: audit-ready claims and unwritten deviations surfaced
mood: pedantic            # optional — a tone/severity dial, not an accuracy boost
filter:
  tags_any: [regulatory, validation, contamination, risk]   # <- must come from config/tags.yaml
```
...
[`examples/biotech/`](examples/biotech/) as the template for a self-contained, shippable example.
...
fixtures (note: the fixtures are the Dune scenario, so the offline *card content* is always Dune —
the retarget proves out in ingestion, tagging, and the persona panel; a real card needs a live key).
